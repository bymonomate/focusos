...
export default function FocusOS() {
  ...
  if (isLivePage) {
    return <FocusLivePage sessions={liveSessions} lang={lang} isJoined={Boolean(joinedLiveSession && getRemainingSeconds(joinedLiveSession) > 0)} joinedSession={joinedLiveSession} comments={liveComments} currentNickname={nickname || anonymousName} onJoin={joinLiveFocus} onLeave={leaveLiveFocus} onSendComment={sendLiveComment} onBack={goToHome} onPlanner={goToPlanner} session={session} onOpenAuth={() => setShowAuth(true)} onOpenProfile={() => setProfileOpen(true)} onOpenSettings={() => setSettingsOpen(true)} onSignOut={signOut} t={t} getRemainingSeconds={getRemainingSeconds} formatRemainingLabel={formatRemainingLabel} />;
  }
      ...
      <div className="sticky top-0 z-30 border-b border-white/70 bg-white/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3 md:px-6">
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            {!session ? (
              <button onClick={() => setShowAuth(true)} className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 shadow-sm transition hover:bg-zinc-50">{t('로그인')}</button>
            ) : null}
            {session ? (<>
          ...
          <div className="border-t border-zinc-200 bg-white px-4 py-3 md:hidden">
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2">
                ...
                {!session ? (
                  <button onClick={() => { setShowAuth(true); setMenuOpen(false); }} className="rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-left text-sm font-medium text-zinc-700 transition hover:bg-zinc-50">{t('로그인')}</button>
                ) : null}
                {session ? (<>
